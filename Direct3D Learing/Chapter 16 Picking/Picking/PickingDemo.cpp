...
void PickingApp::BuildMeshGeometryBuffers()
{
	...
	mMeshIndices.resize(mMeshIndexCount);   //存储于本地，用于检测网格
...
void PickingApp::Pick(int sx, int sy)
{
	...
	// 根据投影矩阵计算出射线检测方向
	float vx = (+2.0f*sx/mClientWidth  - 1.0f)/XMVectorGetX(P.r[0]);
	float vy = (-2.0f*sy/mClientHeight + 1.0f)/XMVectorGetY(P.r[1]);
	...
	XMMATRIX invView = XMMatrixInverse(&XMMatrixDeterminant(V), V);      //求出观察矩阵的逆

	XMMATRIX W = XMLoadFloat4x4(&mMeshWorld);     //网格体的世界坐标
	...
	rayDir = XMVector3TransformNormal(rayDir, toLocal);    //忽略 w 分量
