...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	HR(CreateDDSTextureFromFile(device,
		L"Textures/TerrainArray.dds", 0, &mLayerMapArraySRV, 0));

	HR(CreateDDSTextureFromFile(device, 
		mInfo.BlendMapFilename.c_str(), 0, &mBlendMapSRV, 0));
}
...
void Terrain::BuildHeightmapSRV(ID3D11Device* device)
{
    ...
    texDesc.MipLevels = 1;
	texDesc.ArraySize = 1;
	texDesc.Format    = DXGI_FORMAT_R16_FLOAT;  //以16位的格式创建Texture
	...
	std::transform(mHeightmap.begin(), mHeightmap.end(), hmap.begin(), XMConvertFloatToHalf);  //把mHeightmap中的数据转换成16位，然后转到hmap中，这样就节省了空间
