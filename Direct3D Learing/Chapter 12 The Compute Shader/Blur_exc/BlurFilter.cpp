...
void BlurFilter::Init(ID3D11Device* device, UINT width, UINT height, DXGI_FORMAT format)
{
    ...
    blurredTexDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;       //无序访问视图，就是创建可读写资源
    blurredTexDesc.CPUAccessFlags = 0;      
    blurredTexDesc.MiscFlags      = 0;

	ID3D11Texture2D* blurredTex = 0;
	HR(device->CreateTexture2D(&blurredTexDesc, 0, &blurredTex));       //创建Texture2D
...
//输入一个用于输入到shader的资源，还有一个用于输出的资源
							 ...
							 int blurCount)
{
	...
	{
		// 学过图像处理的可以知道，高斯滤波器可以分离
		// HORIZONTAL blur pass. 横向混合
		D3DX11_TECHNIQUE_DESC techDesc;
		Effects::BlurFX->HorzBlurTech->GetDesc( &techDesc );
		for(UINT p = 0; p < techDesc.Passes; ++p)
		{
			Effects::BlurFX->SetInputMap(inputSRV);      //此时输入的是未经高斯模糊的渲染画面
			Effects::BlurFX->SetOutputMap(mBlurredOutputTexUAV);     //输出经过一次横向滤波指向的资源
			Effects::BlurFX->HorzBlurTech->GetPassByIndex(p)->Apply(0, dc);

			// How many groups do we need to dispatch to cover a row of pixels, where each
			// group covers 256 pixels (the 256 is defined in the ComputeShader).
			UINT numGroupsX = (UINT)ceilf(mWidth / 256.0f);       //由于每个线程组处理256个像素（1维），所以横向X线程组有 mWidth / 256.0f
			dc->Dispatch(numGroupsX, mHeight, 1);        //分配线程组    线程组是一维，所以线程分配纵向Y就是 mHeight
		}
		...
		// VERTICAL blur pass. 纵向混合
		Effects::BlurFX->VertBlurTech->GetDesc( &techDesc );
		for(UINT p = 0; p < techDesc.Passes; ++p)
		{
			Effects::BlurFX->SetInputMap(mBlurredOutputTexSRV);       //输入经过一次
